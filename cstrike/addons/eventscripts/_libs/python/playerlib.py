...
# Eye angle property
def _getEyeAngle():
   if _gamename in ('cstrike', 'left4dead'):
      return 'CCSPlayer.m_angEyeAngles[%s]'
   elif _gamename == 'dod':
      return 'CDODPlayer.m_angEyeAngles[%s]'
   elif _gamename == 'hl2mp':
      return 'CHL2MP_Player.m_angEyeAngles[%s]'
   elif _gamename == 'tf':
      return 'CTFPlayer.tfnonlocaldata.m_angEyeAngles[%s]'
   elif _gamename in ('hl2mp', 'ageofchivalry', 'diprip', 'gesource', 'hl2ctf',
    'sourceforts', 'synergy', 'ZombieMod Source', 'zombie_master', 'zps'):
      return 'CHL2MP_Player.m_angEyeAngles[%s]'
...
class Player(object):
    ...
    def getViewAngle(self):
        ...
        if _eyeangle.count('%s') != 1:
            raise NotImplementedError, 'viewVector goes not support game "' + _gamename + '"'
    ...
    def getDistance(self, value, plane='xyz'):
        ...
        if hasattr(value, '__iter__'):
            ...
            his_x, his_y, his_z = map(float, value)
        else:
    ...
    def isObstructionBetween(self, value):
        ...
        view_location = map(float, self.getViewCoord().split(','))
        result = True
        for x in (0, 1, 2):
    ...
    def set(self, param, value=""):
        ...
        # SET WEAPON COLOR
            ...
            self.setWeaponColor(*map(int, value))
        # SET BURN
        elif param == "burn":
            self._burn()
        # SET EXTINGUISH
    ...
    if not filtername is None:
        ...
        for y in filtername.split(","):
            ...
            if filtername.startswith('#'):
               playerlist = filter(filters[y[1:]], playerlist)
            else:
               playerlist = filter(lambda x: not filters[y[1:]](x), playerlist)
...
def nearCoord((x1, y1, z1), distance, filtername=None):
    """
