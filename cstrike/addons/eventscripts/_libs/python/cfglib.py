...
class AddonCFG(object):
   ...
   def execute(self, queuecmd=False):
      """ Executes the config """
      if self.cfgpath.startswith('%s/cfg/' % self.gamedir):  
         if queuecmd:
            es.server.queuecmd('exec ' + self.cfgpath.replace('%s/cfg/' %self.gamedir, '', 1))
            return
         es.server.cmd('exec ' + self.cfgpath.replace('%s/cfg/' %self.gamedir, '', 1))            
      elif queuecmd:
   ...
   def _parse(self):
      ...
      cfglines = map(str.strip, cfgfile.readlines())
      cfgfile.close()
...
class AddonINI(ConfigObj):
   ...
   def setInitialComments(self, comment_list):
      ...
      if isinstance(comment_list, basestring):
         comment_list = [comment_list,]
      if comment_list and not comment_list[~0]:
         del comment_list[~0]
      self.initial_comment = map(self.formatComment, comment_list)
   ...
   def setFinalComments(self, comment_list):
      ...
      if comment_list and not comment_list[~0]:
         del comment_list[~0]
      self.final_comment = [''] + map(self.formatComment, comment_list)
   ...
   def setGroupComments(self, header, comment_list):
      ...
      if isinstance(comment_list, basestring):
         comment_list = (comment_list,)
      self.comments[header] = [''] + map(self.formatComment, comment_list)
